...
struct connection {
    ...
    boost::asio::awaitable<void> query(const std::string &q, bool single_row_mode = false) const {
        ...
        while (1) {
            ...
            co_await send_query(q, single_row_mode, &res);
            /*if (res) {
...
private:
    boost::asio::awaitable<void> send_query(const std::string &q, bool single_row_mode = false, PGresult **) const {
