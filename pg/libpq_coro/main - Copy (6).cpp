...
struct yield_awaitable {
    ...
    struct my_coro {
        ...
        boost::asio::awaitable<int> next() {
            co_return i++;
        }
    };
    auto coro() {
        return my_coro{ std::move(*frame_) };
    }
...
boost::asio::awaitable<void> async2() {
    ...
    auto coro = query2().coro();
    int val;
    val = co_await coro.next();

    while (auto v = co_await query2()) {
        std::cerr << v << "\n";
    }
    std::cerr << "end" << "\n";
