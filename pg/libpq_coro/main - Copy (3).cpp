...
private:
    ...
    explicit yield_awaitable(yield_awaitable_frame<ReturnType, YieldType, Executor> *a) : frame_(a) {
    }
...
struct yield_awaitable_frame<void, YieldType, Executor> : boost::asio::detail::awaitable_frame_base<Executor> {
    yield_awaitable<void, YieldType, Executor> get_return_object() {
        this->coro_ = std::coroutine_handle<yield_awaitable_frame>::from_promise(*this);
        return yield_awaitable<void, YieldType, Executor>(this);
    };
...
boost::asio::awaitable<void> mygen() {
